...
void calc_spline( const state_type &x_d, const state_type &t_d, const state_type &cub_coeff, const int &ind, state_type &x_spline)
{
     ...
     // t_end + 1 is the final limit
     for (int t_ind = 1; t_ind < (t_d[N_time_points-1]+1); t_ind++ )
     {
          double t_t = 0.0; 
          if ( (double(t_ind) > 0.0) && (double(t_ind) <= t_d[1]) )
          {
             t_t = double(t_ind);
          ...
          else if ( (double(t_ind) > t_d[1]) && (double(t_ind) <= t_d[2]) )
          {
             t_t = double(t_ind)-t_d[1];
          ...
          else if ( (double(t_ind) > t_d[2]) && (double(t_ind) <= t_d[3]) )
          {
             t_t = double(t_ind)-t_d[2];
          ...
          else if ( (double(t_ind) > t_d[3]) && (double(t_ind) <= t_d[4]) )
          {
             t_t = double(t_ind)-t_d[3];
          ...
          else if ( (double(t_ind) > t_d[4]) && (double(t_ind) <= t_d[5]) )
          {
             t_t = double(t_ind)-t_d[4];
          ...
          else if ( (double(t_ind) > t_d[5]) && (double(t_ind) <= t_d[6]) )
          {
             t_t = double(t_ind)-t_d[5];
          ...
          else if ( (double(t_ind) > t_d[6]) && (double(t_ind) <= t_d[7]) )
          {
             t_t = double(t_ind)-t_d[6];
          ...
          else if ( (double(t_ind) > t_d[7]) && (double(t_ind) <= t_d[8]) )
          {
             t_t = double(t_ind)-t_d[7];
          ...
          else if (double(t_ind) > t_d[8]  )
          {
             t_t = double(t_ind)-t_d[8];
   ...
   for ( int ind_i=0; ind_i < N_gene; ind_i++)
   {
       for ( int ind_j=0; ind_j < N_gene; ind_j++)
       {
            if (ind_i != ind_j)
            {
               ...
               //vector <double> mean_xi,sd_xi;
               //calc_stats(x_spline_i,mean_xi,sd_xi); 
               start = ind_j*size_vec;
               end = ((ind_j + 1) * size_vec) - 1;
               vector <double> x_spline_j ( x_spline.begin () + start, x_spline.begin () + end );
               //vector <double> mean_xj,sd_xj;
               //calc_stats(x_spline_j,mean_xj,sd_xj); 
               ...
               //transform(x_spline_i.begin(),x_spline_i.end(),x_spline_j.begin(),corr_vector.begin(),corr_functor(mean_xi[0],mean_xj[0]));
               double corr_ij;
               corr_ij = accumulate(corr_vector.begin(),corr_vector.end(),0.0)/(x_spline_i.size()*sd_xd[ind_i]*sd_xd[ind_j]);
               //corr_ij = accumulate(corr_vector.begin(),corr_vector.end(),0.0)/(x_spline_i.size()*sd_xi[0]*sd_xj[0]);
               correlation.push_back(corr_ij);               
            }
            // correlation for ii or jj is 0.0
            else correlation.push_back(0.0); 
       }
   }
} 
