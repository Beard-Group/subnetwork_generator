...
public:
    ...
    HOST DEVICE
    double get_co0 (int i, int j) const
    {
        return _Dcoeff[i*N_time_points*4+j*4];
    };
    HOST DEVICE
    double get_co1 (int i, int j) const
    {
        return _Dcoeff[i*N_time_points*4+j*4+1];
    };
    HOST DEVICE
    double get_co2 (int i, int j) const
    {
        return _Dcoeff[i*N_time_points*4+j*4+2];
    };
    HOST DEVICE
    double get_co3 (int i, int j) const
    {
        return _Dcoeff[i*N_time_points*4+j*4+3];
    };
    ...
    __device__ void operator()(int *neq, double *t, double *y, double *ydot)
    {
        ...
        if ( *t - 1.0 <= 0.0 ) {
            while ( ka_index < _Dnka[thread_id] ) {
                num += (get_co0(_Dkavec[_Dkastart[thread_id]+ka_index],
                                0) * _Dkaval[_Dkastart[thread_id]+ka_index]);
                ka_index++;
            }
            num *= num*num*num;
            num += ea_ptr[thread_id]*ea_ptr[thread_id]*ea_ptr[thread_id]*ea_ptr[thread_id];
            while ( kd_index < _Dnkd[thread_id] ) {
                den += (get_co0(_Dkdvec[_Dkdstart[thread_id]+kd_index],
                                0) * _Dkdval[_Dkdstart[thread_id]+kd_index]);
                kd_index++;
            }
            den *= den*den*den;
            den += (num + 1.0);
            ...
            while ( ka_index < _Dnka[thread_id] ) {
                num += ((get_co0(_Dkavec[_Dkastart[thread_id]+ka_index],lower_index) +
                         (t_t)*(get_co1(_Dkavec[_Dkastart[thread_id]+ka_index],lower_index)+
                                ((t_t)*(get_co2(_Dkavec[_Dkastart[thread_id]+ka_index],
                                                lower_index)+get_co3(_Dkavec[_Dkastart[thread_id]+ka_index],
                                                        lower_index)*(t_t)))))
            ...
            while ( kd_index < _Dnkd[thread_id] ) {
                den += ((get_co0(_Dkdvec[_Dkdstart[thread_id]+kd_index],lower_index) +
                         (t_t)*(get_co1(_Dkdvec[_Dkdstart[thread_id]+kd_index],lower_index)+
                                ((t_t)*(get_co2(_Dkdvec[_Dkdstart[thread_id]+kd_index],
                                                lower_index)+get_co3(_Dkdvec[_Dkdstart[thread_id]+kd_index],
                                                        lower_index)*(t_t)))))
...
struct myJex {
    __device__ void operator()(int *neq, double *t, double *y, int ml, int mu,
