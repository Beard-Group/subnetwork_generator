...
__device__ int dlsoda_(Fex, int *, double *, double *, double *, int *, double *, double *, int *, int *, int *, double *, int *, int *, int *, Jex, int *, struct cuLsodaCommonBlock *);
 ...
 template<typename Fex, typename Jex>
__global__ void cuLsoda(Fex fex, int *neq, double *y, double *t, double *tout, int *itol, double *rtol, double *atol, int *itask, int *istate, int *iopt, double *rwork, int *lrw, int *iwork, int *liw, Jex jac, int *jt, struct cuLsodaCommonBlock *common, int *err, int probSize)
{
	int me = threadIdx.x + blockIdx.x * blockDim.x;
	err[me] = dlsoda_(fex, &neq[me], &y[me], &t[me], &tout[me], &itol[me], &rtol[me], &atol[me], &itask[me], &istate[me], &iopt[me], &rwork[38*me], &lrw[me], &iwork[21*me], &liw[me], jac, &jt[me], &common[me]);
	__syncthreads();
}
